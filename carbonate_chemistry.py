...
from chemistry_functions import generate_correlation_heatmap, config_figure
...
VARIABLE_PALETTES = {
    ...
    "ta_umol_kg": {
        "base": '#FF8AFF',
        "shades": [
            '#FF8AFF',"#800080", '#FF5CFF', '#FF2EFF', '#FF00FF',
    ...
    st.markdown(
        ...
        <div class="header" style="display: flex; justify-content: space-between; font-family:Roboto; align-items: center; background-color: #408a93; padding: 0px;">
            <div style="font-size: 1.4rem; font-weight: bold; color: white; margin-left: 0px;">
                 State of the Ocean: Carbon Chemistry
            </div>
            <div style="font-size: 1rem; font-weight: bold; font-family:Roboto; margin-right: 100px;margin-top: 10px">
                <a href="https://chem_data.ocean.gov.za/" target="_blank" style="color: white; text-decoration: none;">
                    Access Historical
                </a>
            </div>
        </div>
...
def create_map(df):
    ...
    # Group by station to get one coordinate per station
    df = (
        df.groupby("grid")
    ...
    fig = px.scatter_map(
        ...
        hover_name="grid", zoom=4.7, height=550, map_style="open-street-map", 
        center={"lat": -32.0, "lon": 18.0},
    )
    fig.update_layout(
...
def get_unique_options(chem_data):
    """Extract unique filter options from chem_data"""
    years = []
    try:
        years = sorted(chem_data["datetime"].dt.year.dropna().unique().astype(int))
    except Exception as e:
        st.error(f"Error processing years: {e}")
    
    return {
        "grids": list(chem_data["grid"].unique()),
        "seasons": list(chem_data["season"].unique()),
        "years": years
    }
...
# Tab Components
# ------------------------------
def render_ctd_tab(chem_data):
    """CTD Profiles Tab"""
    st.header("CTD Profiles")
    
    # Get filter options and year range
    options = get_unique_options(chem_data)
    year_range = get_year_range(options["years"])
    
    with st.expander("Filter chem_data"):
        cols = st.columns(4)
        with cols[0]:
        ...
        with cols[2]:
            sel_years = st.multiselect("Year", options["years"], default=[options["years"][1]], key="ctd_years")
        with cols[3]:
    ...
    try:
        df_ctd = chem_data[
            chem_data["grid"].isin(sel_grids) &
            chem_data["season"].isin(sel_seasons) &
            chem_data["datetime"].dt.year.isin(sel_years)
        ].copy()
    ...
    if df_ctd.empty:
        st.warning("No chem_data matches current filters")
        return
...
def render_regression_tab(chem_data):
    ...
    with st.expander("Filter chem_data"):
        ...
        with cols[2]:
            sel_years = st.multiselect("Year", options["years"], default=[options["years"][1]], key="reg_years")
    ...
    with col2:
        ...
        for (station, season, year), group in df_reg.groupby(["grid", "season", "year"]):
            color = generate_station_color(x_var, station, year, year_range)
            ...
            if add_trend == "Yes":
                try:
                    ...
                    st.error(f"Error calculating regression for {station}-{season}-{year}: {e}")
...
def render_correlation_tab(chem_data):
    ...
    with st.expander("Filters", expanded=False):
        ...
        with cols[2]:
            sel_year = st.selectbox("Year", options["years"], index=1, key="heat_year")

    # Filter chem_data
    try:
        df_heat = chem_data[
            (chem_data["grid"] == sel_grid) &
            (chem_data["season"] == sel_season) &
            (chem_data["datetime"].dt.year == sel_year)
        ]
    except Exception as e:
        st.error(f"Error filtering data: {e}")
        return
...
def render_timeseries_tab(chem_data):
    """Time Series Analysis Tab"""
    st.header("Time Series Analysis")
    
    if "datetime" not in chem_data.columns:
    ...
    with st.expander("Filters", expanded=False):
        ...
        with cols[1]:
            start_date = st.date_input("Start Date", value=chem_data["datetime"].min().date())
        with cols[2]:
            end_date = st.date_input("End Date", value=chem_data["datetime"].max().date())
    ...
    try:
        df_ts = chem_data[
            (chem_data["grid"].isin(sel_stations)) &
            (chem_data["datetime"] >= pd.to_datetime(start_date)) &
            (chem_data["datetime"] <= pd.to_datetime(end_date))
        ]
    ...
    with col2:
        ...
        try:
            df_ts[selected_vars] = df_ts[selected_vars].apply(pd.to_numeric, errors='coerce')
            ts_data = df_ts.groupby(["datetime", "grid", "year"])[selected_vars].mean().reset_index()
            ...
            for i, var in enumerate(selected_vars):
                for station in ts_data["grid"].unique():
                    ...
                    year = station_data.iloc[0]["year"]  # Get representative year
            ...
            fig.update_layout(
                ...
                margin=dict(t=60, r=10, b=20, l=40)
            )
...
def load_and_prepare_data():
    ...
    try:
        raw_data = load_data("data/combined_data.csv")
        raw_data = raw_data.rename(columns={
            'Grid No': 'grid',
            'Date': 'datetime',
            'Latitude': 'latitude',
            'Longitude': 'longitude',
            'Pressure [db]': 'pressure_db',
            'Temp [°C-T090]': 'temperature_c',
            'DIC [umol/kg]': 'dic_umol_kg',
            'TA [umol/kg]': 'ta_umol_kg',
        })
        ...
        raw_data['depth_m'] = raw_data['pressure_db']  # Convert pressure to depth in meters
        raw_data['season'] = raw_data['datetime'].dt.month.map({
        ...
        numeric_cols = [
            "pressure_db", "temperature_c", "dic_umol_kg",
            "depth_m", "ta_umol_kg", "latitude", "longitude",
        ]
...
def main():
    ...
    if 'chem_data' not in st.session_state:
        st.session_state.chem_data = load_and_prepare_data()
    ...
    with tab1:
        render_ctd_tab(chem_data)
    with tab2:
        render_regression_tab(chem_data)
    with tab3:
        render_correlation_tab(chem_data)
    with tab4:
        render_timeseries_tab(chem_data)
